...
"""
Mac-safe smoke test for Pixtral integration.

This test monkeypatches Pixtral model/processor loading with small local dummy
...
from rv_train.models.pixtral.model import PixtralActor
...
def run_smoke_test():
    ...
    PixtralActor.load_pixtral_model = staticmethod(
        lambda **kwargs: DummyModel(vocab_size=len(DummyTokenizer().tok_to_id))
    )
    PixtralActor.load_pixtral_processor = staticmethod(
        lambda **kwargs: DummyProcessor()
    )

    model = PixtralActor(
        pixtral_model_id="mistralai/Pixtral-12B-2409",
