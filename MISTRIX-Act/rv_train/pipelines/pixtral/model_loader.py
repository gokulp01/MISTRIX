...
class PixtralModelLoader:
    ...
    def _require_transformers():
        if AutoProcessor is None or AutoModelForCausalLM is None:
            raise ImportError(
                "transformers is required for Pixtral support. "
                "Install with pixtral extras."
            )
    ...
    def restore_from_checkpoint(
        ...
        if actor.use_lora:
            ...
            base_model = cls.create_model(
                model_id=actor.pixtral_model_id,
            ...
            print("Loading Pixtral PEFT model from", path)
        else:
            dtype = torch.bfloat16 if torch.cuda.is_available() else torch.float32
            extra_kwargs = cls._extra_model_kwargs(
            ...
            actor.model = model
            print("Loading Pixtral full model from", path)
        ...
        print("Loading Pixtral processor from", path)
