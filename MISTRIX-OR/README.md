...
1. [What MISTRIX-OR Does](#what-mistrix-or-does)
...
## What MISTRIX-OR Does

MISTRIX-OR is an audio-guided surgical instrument localization pipeline.
...
- **Instrument classification**: Pixtral embeddings + saved scikit-learn classifier head.
...
6. Load classifier artifacts + Pixtral backbone.
...
MISTRIX-OR model artifacts are distributed from the **same Google Drive as MISTRIX-Rx**:

- `https://drive.google.com/drive/folders/1kkkcYwWtJQnKJdtWwpgQccnoZkVDLx7W?usp=sharing`

Place MISTRIX-OR classifier artifacts under:

- `surgical_tool_pipeline/models/best_model/`

Expected MISTRIX-OR model directory layout:
...
- `--model_id_override`: override Pixtral backbone ID
...
```bash
python -m surgical_tool_pipeline.pixtral_infer \
...
### `surgical_tool_pipeline.pixtral_infer`

```bash
python -m surgical_tool_pipeline.pixtral_infer [flags]
```
...
- Re-download MISTRIX-OR model artifacts from the shared Google Drive.

### `Failed to load Pixtral backbone`
...
python -m surgical_tool_pipeline.pixtral_infer --help
```
...
- If you retrain the classifier head, keep artifact schema consistent with:
  ...
  - `label_mapping.json`
  - `candidate_results.json`
- `surgical_tool_pipeline.pixtral_infer` uses a legacy default model dir (`outputs/models/best_model`), so pass `--model_dir surgical_tool_pipeline/models/best_model` unless you intentionally store artifacts elsewhere.
