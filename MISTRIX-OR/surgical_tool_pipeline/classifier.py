"""Crop classification helpers backed by Pixtral embeddings and a saved classifier."""
...
def predict_crops(
    ...
    device = pixtral_infer.get_device(device_name)
    ...
    try:
        processor = _from_pretrained_with_optional_token(AutoProcessor, model_id)
        backbone = _from_pretrained_with_optional_token(AutoModel, model_id).to(device).eval()
    except OSError as exc:
        raise RuntimeError(
            "Failed to load Pixtral backbone. If using gated repo access, set one of "
            "HF_TOKEN/HUGGINGFACE_HUB_TOKEN/HUGGING_FACE_HUB_TOKEN in the shell, or run "
            "`huggingface-cli login`, then retry."
        ) from exc

    dataset = pixtral_infer.SafeFileImageDataset(crop_paths)
    X, valid_paths, skipped = pixtral_infer.run_embedding_extraction(
        dataset=dataset,
        processor=processor,
        model=backbone,
        device=device,
    ...
    probs = pixtral_infer.predict_proba_safe(classifier, X)
