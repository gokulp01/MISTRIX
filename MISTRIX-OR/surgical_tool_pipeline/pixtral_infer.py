...
def parse_args() -> argparse.Namespace:
    """Parse standalone Pixtral inference script arguments."""
    parser = argparse.ArgumentParser(
        description="Standalone folder inference script for Pixtral embeddings + saved classifier."
    )
...
def _build_processor_inputs(processor: Any, images: List[Image.Image]) -> Dict[str, Any]:
    """Build processor inputs for both image-only and image+text VLM processors."""
    try:
        return processor(images=images, return_tensors="pt")
    except TypeError:
        prompts = ["Describe the primary surgical instrument in the image."] * len(images)
...
def run_embedding_extraction(
    ...
    for batch in tqdm(loader, desc="Embedding input images"):
        ...
        inputs = _build_processor_inputs(processor=processor, images=images)
        inputs = {
...
def predict_proba_safe(classifier: Any, X: np.ndarray) -> np.ndarray:
    ...
    if expected_dim is not None and X.shape[1] != int(expected_dim):
        raise RuntimeError(
            f"Classifier expects feature dim {int(expected_dim)}, but extracted embeddings have dim {X.shape[1]}. "
            "Rebuild/retrain classifier artifacts for the selected Pixtral backbone."
        )
...
def main() -> None:
    ...
    logger.info("Loading image processor + Pixtral backbone: %s", model_id)
