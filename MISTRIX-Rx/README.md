...
1. [What MISTRIX-Rx Does](#what-mistrix-rx-does)
...
## What MISTRIX-Rx Does

MISTRIX-Rx provides two connected layers:
...
- **Backend (`backend/`)**: modular image pipeline.
  ...
  - Runs Pixtral 12B fine-tuned inference.
...
Backend inference expects Pixtral fine-tuning artifacts saved from `finetuning/train.py` in one of these layouts:
...
Recommended location is under `finetuning/outputs/<run_name>/`.

You can point pipeline explicitly with `--run_dir`, or set env var:

```bash
export MISTRIX_RX_MODEL_RUN_DIR=/absolute/path/to/finetuning/outputs/<run_name>
```

If `--run_dir` is not provided, backend auto-searches `finetuning/outputs/` for the newest compatible run.
...
- `--mode`: `pixtral` (legacy aliases `contrastive|linear_probe|partial_unfreeze|lora_optional` are remapped)
...
- `--mode` in `pixtral|contrastive|linear_probe|partial_unfreeze|lora_optional`
...
- `--mode` controls inference mode (Pixtral is primary; legacy names are aliases)
...
You can force a specific finetuned run path with `MISTRIX_RX_MODEL_RUN_DIR=/abs/path/to/run_dir`.
