...
from backend.infer_core import run_pixtral_inference
...
def run_labeler(
    ...
    """Run Pixtral inference over crops and persist JSONL prediction artifacts."""
    resolved_mode = resolve_mode(run_dir, mode)
    ensure_text_tokenizer_deps_available(resolved_mode)

    infer_candidates = _load_candidates(prescription_file, prescriptions, prescriber_json)
