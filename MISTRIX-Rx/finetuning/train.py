...
def ensure_processor_padding(processor, model=None) -> None:
...
def _decode_generation_tails(
    ...
    for i in range(generated_ids.size(0)):
        ...
        if prompt_len < len(ids):
            decode_ids = ids[prompt_len:]
        else:
            # Some Pixtral generation paths return only newly generated tokens.
            decode_ids = ids
        text = decoder.decode(decode_ids, skip_special_tokens=True).strip()
        prompt_text = prompts[i] if prompts is not None and i < len(prompts) else None
        text = _strip_prompt_prefix(text, prompt_text)
...
def evaluate_generation(
    ...
    for batch in pbar:
        ...
        for k, v in batch.items():
            if k in {"label_indices", "label_texts", "image_paths", "prompt_lengths", "labels", "prompts"}:
                continue
        ...
        decoded = _decode_generation_tails(
            processor,
            generated,
            prompt_lengths,
...
def run_training(args: argparse.Namespace, run_dir: Path, splits, labels: list[str]) -> None:
    ...
    ensure_processor_padding(processor, model=model)
