...
If OpenCV has an architecture mismatch such as `have 'arm64', need 'x86_64'`, MISTRIX-RX falls back to a non-OpenCV OCR parser so the UI keeps working. For best table-extraction accuracy, rebuild the environment:
```bash
rm -rf .venv
uv venv --python 3.11
uv sync
...
If you place images in `image-input/` (repository root), MISTRIX-RX prioritizes the newest image from that folder for pipeline runs.
