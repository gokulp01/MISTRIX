#!/usr/bin/env python3
"""MISTRIX RX web UI for prescription parsing and slash-command workflows."""
...
def apply_styles() -> None:
    """Inject custom CSS theme for the MISTRIX RX console."""
    st.markdown(
        """
<style>
...
def generate_med_image_for_target(target_medicine: str) -> Path:
    ...
    model_run_dir = os.environ.get("MISTRIX_RX_MODEL_RUN_DIR", "").strip()
    if model_run_dir:
        cmd.extend(["--run_dir", model_run_dir])
    env = os.environ.copy()
...
def main() -> None:
    ...
    st.set_page_config(page_title="MISTRIX RX Nurse UI", layout="wide")
    apply_styles()
    init_state()
    ...
    logo_candidates = [
        FRONTEND_ASSETS_DIR / "mistrixrx.png",
        FRONTEND_ASSETS_DIR / "mistrixrx.jpg",
        FRONTEND_ASSETS_DIR / "mistrixrx.jpeg",
        POSTOP_ROOT / "mistrixrx.png",
        POSTOP_ROOT / "mistrixrx.jpg",
        POSTOP_ROOT / "mistrixrx.jpeg",
    ]
    ...
    with header_right:
        st.title("MISTRIX RX Nurse Console")
